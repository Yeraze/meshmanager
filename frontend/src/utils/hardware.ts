...
/**
 * Get human-readable hardware model name from numeric ID or string
 * @param hwModel - Numeric hardware model ID or string representation
 * @returns Readable hardware model name or 'Unknown' if not found
 */
export function getHardwareModelName(hwModel: number | string | undefined | null): string {
  if (hwModel === undefined || hwModel === null) {
    return 'N/A'
  }

  // Convert string to number if needed
  const modelNum = typeof hwModel === 'string' ? parseInt(hwModel, 10) : hwModel

  if (isNaN(modelNum)) {
    // If it's a non-numeric string, return it as-is (might already be a name)
    return String(hwModel)
  }

  return HARDWARE_MODELS[modelNum] || `Unknown (${modelNum})`
}

/**
 * Format hardware model name for display
 * Converts technical names to more readable format
 * @param hwModel - Numeric hardware model ID or string representation
 * @returns Formatted hardware model name
 */
export function formatHardwareModelName(hwModel: number | string | undefined | null): string {
